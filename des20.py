...
SHIFT = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1, 1, 2, 2, 2]


def string_to_bit_array(text):  # Convert a string into a list of bits
    array = list()
    for char in text:
        binval = binvalue(char, 4)  # Get the char value on one byte
        array.extend([int(x) for x in list(binval)])  # Add the bits to the final list
    return array


def bit_array_to_string(array):  # Recreate the string from the bit array
    res = ''.join([hex(int(y, 2))[2:] for y in [''.join([str(x) for x in bytes]) for bytes in nsplit(array, 4)]])
    while len(res) < 16:
        res = "0" + res
    return res
...
def binvalue(val, bitsize):  # Return the binary value as a string of the given size
    if isinstance(val, int):
        binval = bin(val)[2:]
    elif (ord(val) > 47 and ord(val) < 58):
        binval = bin(ord(val) - 48)[2:]
    elif (val == 'a' or val == 'A'):
        binval = bin(10)[2:]
    elif (val == 'b' or val == 'B'):
        binval = bin(11)[2:]
...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        for block in text_blocks:  # Loop over all the blocks of data
            ...
            tmp = None
            for i in range(20):  # Do the 16 rounds
                d_e = self.expand(d, E)  # Expand d to match Ki size (48bits)
                if action == ENCRYPT:
                    tmp = self.xor(self.keys[i], d_e)  # If encrypt use Ki
                else:
                    tmp = self.xor(self.keys[19 - i], d_e)  # If decrypt start by the last key
    ...
    def generatekeys(self):  # Algorithm that generates all the keys
        ...
        for i in range(20):  # Apply the 16 rounds
